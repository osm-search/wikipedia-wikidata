...
do
    ...
    zcat downloaded/${LANG}/page.sql.gz | \
    ...
    grep -e ',0$' | \
    sed 's/,0$//' | \
    gzip -9 > converted/$LANG/pages.csv.gz
    ...
    zcat downloaded/${LANG}/pagelinks.sql.gz | \
    python3 bin/mysqldump_to_csv.py | \
    sed 's/\x0//g' | \
    sed 's/\r\?//g' | \
    csvcut -c 3,2 | \
    grep -e ',0$' | \
    sed 's/,0$//' | \
    gzip -9 > converted/$LANG/pagelinks.csv.gz
    ...
    zcat downloaded/${LANG}/langlinks.sql.gz | \
    python3 bin/mysqldump_to_csv.py | \
    sed 's/\x0//g' | \
    sed 's/\r\?//g' | \
    csvcut -c 3,1,2 | \
    gzip -9 > converted/$LANG/langlinks.csv.gz
    ...
    zcat downloaded/${LANG}/redirect.sql.gz | \
    python3 bin/mysqldump_to_csv.py | \
    sed 's/\x0//g' | \
    sed 's/\r\?//g' | \
    csvcut -c 1,3,2 | \
    grep -e ',0$' | \
    sed 's/,0$//' | \
    gzip -9 > converted/$LANG/redirects.csv.gz
done
